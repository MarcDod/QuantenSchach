...
public class Figur {
    ...
    public final static BufferedImage[][] FIGUR_IMAGE = new BufferedImage[2][7];
    ...
    public enum FigurTyp {
        ...
        public FigurTyp getType(int x) {           
            return FigurTyp.values()[x];
        }
            ...
            switch (this) {
                case DAME:
                    ...
                    for (int i = 1; i < ((absX > absY) ? absX : absY); i++) {
                        if (field[stand.y + i * ((absY == 0) ? 0 : dY / absY)][stand.x
                                + i
                ...
                case BAUER:
                    ...
                    if (absX == 1 && (targetFigur == null || targetFigur
                            .getColor() == color)) {
                        return false;
                    }
                    if ((down ? 1 : -1) != dY) {
    ...
    public ArrayList<Point> getMoves(Figur[][] field) {
        return this.typ.getMoves(new Point(x, y), (this.farbe == FIGUR_BLACK),
                field, farbe);
    }

    public boolean validateMove(Point target, Figur[][] field) {
        return this.typ.validateMove(new Point(x, y), target, (this.farbe
                == FIGUR_BLACK), field, farbe);
    }
